  ...
  return (
    <Popover className="relative">
      {({ open }) => (
        <Fragment>
          <Popover.Button
            className={`${sequence.pattern === "bruteforce" &&
          ...
          >
            ...
            >
              <div className="px-3 py-2">
                ...
                {sequence.pattern === "repeat" &&
                  sequence.base_matches.map((item, index) => (
                    <Fragment key={index}>
                ...
                {sequence.pattern === "sequence" && (
                  <Fragment>
                    <div>
                      Sequence name: {_.capitalize(sequence.sequence_name)}
                    </div>
                    <div>
                      Ascending: {sequence.ascending ? "True" : "False"}
                    </div>
                    <div>Total guesses: {sequence.guesses}</div>
                  </Fragment>
                )}
